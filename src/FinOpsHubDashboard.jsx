...
export default function FinOpsHubDashboard() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 font-[DM Sans]">
      <style>{`
      ...
      {/* Main */}
      <main className={`flex-1 transition-all duration-300 ${collapsed ? 'ml-12' : 'ml-60'} overflow-y-auto`}>
        <div className="p-6 gap-6 flex flex-col">
          ...
          <section className="grid grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5 hover:-translate-y-0.5 transition shadow-md cursor-pointer">
              ...
              <button className="mt-3 text-sm text-slate-600 hover:text-slate-800 font-medium">Arrow link →</button>
            </div>

            {/* Card 4: Peer Standing */}
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5 hover:-translate-y-0.5 transition shadow-md cursor-pointer">
              <div className="flex items-start justify-between">
                <div>
                  <div className="text-2xl font-bold text-emerald-600">+13 pts</div>
                  <div className="mt-2 text-sm text-slate-500">
                    Above industry avg <span className="font-medium">(61)</span>
                  </div>
                </div>
          ...
          <section className="grid grid-cols-[35%_65%] gap-4">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between text-slate-700">
                    <span>Rate optimizations</span>
                    <Money value="$14,200/mo" />
                  </div>
                  <div className="flex justify-between text-slate-700">
                    <span>Usage right-sizing</span>
                    <Money value="$8,670/mo" />
                  </div>
                  <div className="flex justify-between text-slate-700">
                    <span>Idle / waste</span>
                    <Money value="$3,100/mo" />
                  </div>
            ...
            {/* RIGHT: AI-Prescribed Optimizations */}
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
              <div className="flex items-center justify-between mb-4">
                    ...
                    {filtered.map((row, idx) => (
                      <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50">
                        ...
                        <td className="py-3 px-3">
                          ...
                          {row.chip && (
                            <div className="inline-block mt-2 px-2 py-1 bg-emerald-100 text-emerald-700 text-xs font-medium rounded">
                              {row.chip} chip
                            </div>
                          )}
                        </td>
                        ...
                        <td className="py-3 px-3 text-center">
                          <button className="px-3 py-1 text-sm bg-slate-100 hover:bg-emerald-50 hover:text-emerald-700 rounded-lg font-medium transition-colors">
                            Apply ↗
                          </button>
                        </td>
                      </tr>
          ...
          <section className="grid grid-cols-[60%_40%] gap-4">
            ...
            {/* RIGHT: Split into 2 subsections */}
            <div className="space-y-4">
              {/* Peer Benchmarks */}
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div key={idx}>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-blue-300 opacity-60"
                              style={{ width: `${bench.avg}%` }}
                            />
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-blue-200 opacity-40"
                              style={{ width: `${bench.top}%` }}
                            />
                          </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-500">14 actions applied</div>
                  </div>
                </div>
                  ...
                  {monthlyApplied.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-emerald-400"
                          style={{ width: `${(item.value / 11300) * 100}%` }}
                        />
                      </div>
                      <span className="text-xs font-medium text-slate-800 w-14 text-right">
                        <Money value={`$${(item.value / 1000).toFixed(1)}K`} />
                      </span>
                    </div>
                  ))}
                </div>
